...
class Commander():
    ...
    def send_formation_setpoint(self, roll, pitch, yawrate, thrust):
        """
        Send a new control setpoint for roll/pitch/yawrate/thrust to the copter

        The arguments roll/pitch/yawrate/trust is the new setpoints that should
        be sent to the copter
        """
        if thrust > 0xFFFF or thrust < 0:
            raise ValueError('Thrust must be between 0 and 0xFFFF')
        ...
        pk.data = struct.pack('<Bfff', TYPE_FORMATION,
