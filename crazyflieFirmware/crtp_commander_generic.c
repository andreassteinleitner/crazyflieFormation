...
struct formationPacket_s {
  struct {
      uint8_t numAuxChannels : 4;   // Set to 0 through MAX_AUX_RC_CHANNELS
      uint8_t reserved : 4;
  } hdr;
  uint16_t channelRoll;		// deg
  uint16_t channelPitch;	// deg
  uint16_t channelYawrate;	// deg/s
  uint16_t channelThrust;
  uint16_t channelAux[MAX_AUX_RC_CHANNELS];
} __attribute__((packed));

static void formationDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  bool isSelfLevelEnabled = true;

  ASSERT(datalen >= 9); // minimum 9 bytes expected - 1byte header + four 2byte channels
  const struct formationPacket_s *values = data;
  ASSERT(datalen == 9 + (2*values->hdr.numAuxChannels)); // Total size is 9 + number of active aux channels
...
const static packetDecoder_t packetDecoders[] = {
  ...
  [formationType]     = formationDecoder,
};
